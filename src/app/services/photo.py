            ...
            async with self.uow:
                ...
                if target_cluster_id == "reserve":
                    ...
                    if not cluster:
                        ...
                        await self.uow.flush() # Need ID for next steps
                ...
                if source_cluster_id != target_cluster_id:
                    ...
                    if src_cluster_obj and src_cluster_obj.name != "reserve":
                        ...
                        # The photo being moved is still "active" in DB until commit, 
                        # but we updated the object instance 'photo' already.
                        # We need to query assuming the move happened or check if 'active_photos' only contains the moving photo.
                        
                        # Since we haven't flushed, the DB still sees the old state.
                        # However, we can check the length of active_photos.
                        # If len is 1 and that one is our photo, then it will be empty.
                        is_empty_after_move = False
                        if len(active_photos) == 0:
                             is_empty_after_move = True
                        elif len(active_photos) == 1 and active_photos[0].id == photo.id:
                             is_empty_after_move = True
