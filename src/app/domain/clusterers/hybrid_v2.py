...
class CosPlaceExtractor:
    ...
    def __init__(self, onnx_path="assets/cosplace_resnet50_int8.onnx"):
        self.use_onnx = False
...
class HybridCluster(Clusterer):
    def __init__(self):
        ...
        self.extractor = CosPlaceExtractor(onnx_path="assets/cosplace_resnet50_int8.onnx")
            ...
            for i in range(0, n_photos, BATCH_SIZE):
                ...
                for idx in batch_indices:
                    if idx in local_files_map and local_files_map[idx].exists():
                        try:
                            img = Image.open(local_files_map[idx])
