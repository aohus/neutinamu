...
async def test_sync_clusters(client):
    job_id = "job_123"
    payload = {"clusters": [{"id": "c1", "name": "Cluster 1", "order_index": 0, "photo_ids": []}]}
...
async def test_create_cluster(client):
    job_id = "job_123"
    payload = {"name": "New Cluster", "order_index": 1, "photo_ids": []}
...
async def test_add_photos_to_cluster(client):
    ...
    with patch("app.api.endpoints.cluster.ClusterService") as MockService:
        ...
        assert response.status_code == status.HTTP_204_NO_CONTENT
        mock_service.add_photos.assert_called_once_with(job_id="", cluster_id=cluster_id, photo_ids=["p1", "p2"])
